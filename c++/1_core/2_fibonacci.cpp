...
unsigned long long fibonacci_iterative_ugly(int n) {
    ...
    if (n == 1 || n == 2) return 1;  // O(1) | O(1)
    std::vector<unsigned long long> fib = {1, 1};  // O(1) | O(1)
    for (int i = 2; i < n; ++i) {  // O(1) | O(n)
        fib.push_back(fib[fib.size() - 2] + fib[fib.size() - 1]);  // O(n) | O(1)
    }
    return fib.back();  // O(1) | O(1)
}
...
unsigned long long fibonacci_functional(int n) {
    ...
    if (n < 0) throw std::invalid_argument("n must be non-negative");  // O(1) | O(1)
    // lazy range of n-1 steps (clamped so n=0 produces an empty range, not an invalid one)
    auto range = std::views::iota(0, std::max(0, n - 1));  // O(1) | O(1)
    // fold the pair-update lambda over the range, starting from (F(2), F(1)) = (1, 1)
